...
func (m *GoCode) CloneVT() *GoCode {
	...
	r := &GoCode{
		...
		EmitEmbedAlias:            m.EmitEmbedAlias,
	}
...
func (this *GoCode) EqualVT(that *GoCode) bool {
	...
	if this.EmitEmbedAlias != that.EmitEmbedAlias {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *GoCode) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 28:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field EmitEmbedAlias", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
